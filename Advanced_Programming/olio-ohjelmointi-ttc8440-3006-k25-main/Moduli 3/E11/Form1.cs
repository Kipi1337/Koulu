...
namespace E10
{
    public partial class Form1 : Form
    {
        ...
        private int muokattavaIndex = -1; // Indeksi muokattavalle autolle
        ...
        // Button click event to add or update a car
        private void BtnLisaa_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(merkki) && !string.IsNullOrEmpty(malli) && !string.IsNullOrEmpty(rekisterinumero))
            {
                ...
                // If we are editing an existing car
                if (muokattavaIndex >= 0)
                {
                    autoManager.PoistaAuto(autoManager.HaeAutot()[muokattavaIndex]); // Remove old car
                    autoManager.LisaaAuto(auto);  // Add updated car
                    muokattavaIndex = -1; // Reset edit index
                }
                else
                {
        ...
        // Button click event to edit the selected car
        private void BtnMuokkaa_Click(object sender, EventArgs e)
        {
            var selectedCar = lstAutot.SelectedItem as Auto;  // Get the selected car from the ListBox
            if (selectedCar != null)
            {
                // Set the textboxes with the selected car's details
                txtMerkki.Text = selectedCar.Merkki;
                txtMalli.Text = selectedCar.Malli;
                txtRekisterinumero.Text = selectedCar.Rekisterinumero;
                muokattavaIndex = lstAutot.SelectedIndex;  // Store the index for editing
            }
            else
            {
                MessageBox.Show("Valitse muokattava auto listalta.");  // Show a message if no car is selected
            }
        }
    }
}
