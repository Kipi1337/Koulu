    ...
    m_bActive(false)
{
    ...
    memset(m_MouseButtonStates, 0, 8);
}
...
bool IApplication::ProcessEvents(XEvent* evnt)
{
    ...
    case ButtonPress:
        ...
        else
        {
            ...
            app->m_MouseButtonStates[touchIndex & 0x7] = 1;
       		app->OnMouseBegin(touchIndex, glm::vec2(evnt->xbutton.x, evnt->xbutton.y));
        }
        break;
    ...
    case ButtonRelease:
        ...
        {
            const int touchIndex = evnt->xbutton.button - 1;
            app->m_MouseButtonStates[touchIndex & 0x7] = 0;
       		app->OnMouseEnd(touchIndex, glm::vec2(evnt->xbutton.x, evnt->xbutton.y));
        }
        break;
    ...
    case MotionNotify:
        {
            ...
            const glm::vec2 mouseCoord(evnt->xbutton.x, evnt->xbutton.y);

            static glm::vec2 previousMouseCoord(mouseCoord);
            app->m_MousePosDelta = mouseCoord - previousMouseCoord;
            previousMouseCoord = mouseCoord;
            app->OnMouseDrag(0, mouseCoord);
        }
        break;
    ...
    case ConfigureNotify:
        if (evnt->xconfigure.width != app->m_iWidth || evnt->xconfigure.height != app->m_iHeight)
        {
            app->m_iWidth = evnt->xconfigure.width;
            app->m_iHeight = evnt->xconfigure.height;
            app->m_pRenderer->SetViewport({0, 0, app->m_iWidth, app->m_iHeight});
            app->OnScreenSizeChanged(app->m_iWidth, app->m_iHeight);
        }
        break;
...
{
    return m_MouseButtonStates[buttonIndex & 0x7];
}
